...
| ~~**M9**~~ | ~~**El personaje se archiva, no se borra.**~~ **CERRADA el 2026-09-04 (2.5.8).** `Character.archivedAt` (`null` = activo); `list()` lo excluye con el mismo patrón que 2D.6 excluye a los PNJ (`statblockRef: null`, un filtro más). `archive()`/`unarchive()` usan `requireEditable` (dueño o DM), no tocan hoja/inventario/dinero, y dejan `CHARACTER_ARCHIVED`/`CHARACTER_RESTORED` en la línea de tiempo. Borrar de verdad sigue existiendo. Ver [07-historial.md](./07-historial.md) | Cerrada |
