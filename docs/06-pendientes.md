...
| **S6** | **La mejora de característica de los niveles 4, 8, 12, 16 y 19 no se modela todavía como elección** (2A.4) | Es el mismo mecanismo que el «+1 a dos» del semielfo, y el plan (§3) las nombra juntas. No entra aquí porque «+2 a una **o** +1 a dos» es una forma de concesión distinta de la que hay —una elección con dos modos— y **quien decide la forma de la subida de nivel es 2A.9**. Hoy un personaje de nivel 5 no recibe su mejora ni avisa de que falta; con 2A.9 lo hará. Añadirla es un `kind` nuevo en `Grant`, no un cambio de lo escrito |
