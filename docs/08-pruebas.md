...
> - **Ficheros de e2e de navegador:** 50 (`apps/web/e2e/*.spec.ts`), contados del disco.
>
...
- **El ataque no se ve resolverse PULSANDO «Atacar» en el navegador, de punta a punta.** El
  ...
  **Desde la tarea 11 del pulido (C4, #15), `combate.spec.ts` sí mide que el resultado LLEGA al
  ...
  Thora»—, pero **monta el ataque por API, no pulsando el botón**: montar el equipo y la tirada
  entera desde el cuadro de ataques de la hoja es el camino de `tirada.spec.ts`, y encadenarlo
  con un combate activo y ese mismo botón sigue sin recorrido. `iniciativa-en-vivo.spec.ts` mide
  pedir y responder la iniciativa, no atacar. `sesion.spec.ts` mide el gesto de anotar daño desde
  el elenco, pero con un número **escrito a mano**, no con el resultado de una tirada de ataque.
