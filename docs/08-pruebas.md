...
> - **Ficheros de e2e de navegador:** 54 (`apps/web/e2e/*.spec.ts`), contados del disco.
>
...
| `conjuros` | **3A.2 (Task 6, T11) — elegir: la pestaña «Conjuros», con un mago hecho por la API** (misma técnica que `combate.spec.ts`/`hoja-pestanas.spec.ts`, para no recorrer el formulario). El mago nace con su libro sembrado (Task 3, D-CF-125): «6 de 6 en el libro» en la cabecera de «Listos para lanzar» antes de tocar nada. Preparar «Proyectil mágico» y «Escudo» desde «Disponibles» sube el contador a «2 de 6 preparados» **y las mueve de zona** —dejan de estar en «Disponibles», aparecen en «Listos para lanzar»—; conocer el truco «Descarga de fuego» sube «1 de 3 trucos» sin tocar el de preparados. Buscar «bola» en «Disponibles» deja una sola fila, «Bola de fuego», marcada **«Fuera del libro»** con el botón «Añadir al libro» — nunca desaparecida, ni deshabilitada por ningún tope. **Capturas** a 1280×800 y 390×844 en `apps/web/e2e-resultados/conjuros-{1280,390}.png`. **Escrito, no corrido** (D-CF-65): la ejecuta el orquestador. |
