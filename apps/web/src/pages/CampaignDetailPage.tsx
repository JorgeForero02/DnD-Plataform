...
function CharactersTab({ campaignId }: { campaignId: string }) {
  ...
  //
  // **Y su error se pinta, que no es un detalle**: sin esto, un fallo de red dejaba `archivados`
  // en `undefined`, el archivo devolvía `null` y el hueco vacío caía en la rama que NO los
  // menciona — o sea, una campaña con todos sus personajes archivados se leía, palabra por
  // palabra, «Ningún personaje todavía». Es la trampa que el plan 06 nombra, entrando por la
  // puerta de al lado. Lo encontró la revisión.
  const { data: archivados, isError: errorArchivo } = useArchivedCharacters(campaignId);
  const {
    role,
    isLoading: roleLoading,
    isError: roleError,
  ...
  return (
    <div>
      ...
      {isError && <p className="text-danger-text">{(error as Error).message}</p>}
      {errorArchivo && (
        <p className="mb-3 text-chrome-xs text-danger-text">
          No se ha podido leer el archivo de personajes. Puede que haya personajes archivados que no
          se están enseñando aquí.
        </p>
      )}
      {data && data.length === 0 && (
