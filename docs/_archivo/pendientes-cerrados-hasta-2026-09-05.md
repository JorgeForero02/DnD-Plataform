...
## ~~P2 · Tres e2e siguen resolviendo un `PREPARING` a mano hasta que exista la tarea 3~~ — CERRADA (2026-09-05, tarea 3)

Desde que `start()` reparte por dueño (tarea 2), varios e2e ya existentes montaban combates
mixtos —un personaje del jugador junto a PNJ del DM— para probar algo que **no** era el reparto:
pasar turno, terminar el combate, que caduque una condición, comparar un ataque. Esos combates
nacían `PREPARING`, y como la tarea 3 (responder la petición de iniciativa y escribir el número
real) todavía no existía, no había ninguna puerta de la API que los subiera a `ACTIVE`. Los que
lo necesitaban llamaban a un puente compartido y explícito, `apps/api/test/helpers/resolver-preparing-a-mano.ts`,
que hacía a mano exactamente lo que esa tarea haría —fijaba una iniciativa real, cerraba las
peticiones, subía el encuentro y escribía `ENCOUNTER_STARTED`— y nunca en silencio: si se llamaba
sobre un encuentro que no tenía ninguna petición pendiente, `expect(pendientes.length).toBeGreaterThan(0)`
lo reventaba.

**La tarea 3 borró el helper entero** y sustituyó sus dos llamadas por el camino real —responder
la petición de verdad por HTTP contra `POST /campaigns/:id/roll-requests/:id/roll`—:

- `apps/api/test/encounters.e2e-spec.ts` — la prueba «se resuelve a mano...» pasó a llamarse
  «responder la iniciativa por HTTP arranca el combate».
- `apps/api/test/ataque-comparado-en-el-servidor.e2e-spec.ts` — en su `beforeAll`, responde la
  única petición pendiente (la del jugador) con `tokenPL` antes de atacar.
