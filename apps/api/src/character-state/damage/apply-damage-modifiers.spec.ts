...
describe("applyDamageModifiers()", () => {
  ...
  // La revisión de cierre del 2026-09-04 tumbó esta prueba, que exigía 25 —cancelación—
  // apoyada en una cita que no existía. La edición INGLESA del SRD dice «Resistance **and
  // then** vulnerability are applied…»; la española pierde el «and then», y de ahí salió el
  // error. Se encadenan: mitad y luego doble.
  //
  // **Y el número tiene que ser impar para que la prueba pruebe algo.** Con 24: mitad 12,
  // doble 24 — idéntico a cancelar, y la prueba pasaría con el código malo. Con 25: mitad
  // 12 (redondeando hacia abajo), doble 24, que es lo único que distingue las dos lecturas.
  it("resistencia y vulnerabilidad al mismo tipo se ENCADENAN: mitad y luego doble", () => {
    ...
    // Y la traza enseña los DOS pasos, en su orden, en vez de un «se cancelan» que esconde
    // la regla: 25 base, −13 por resistir, +12 por vulnerable.
    expect(r.steps.map((p) => [p.op, p.amount])).toEqual([
      ["base", 25],
      ["cap", -13],
      ["add", 12],
    ]);
  ...
  // La otra mitad de la misma regla: con daño PAR las dos lecturas coinciden, y se deja
  // escrito para que nadie "arregle" la de arriba mirando solo este caso.
  it("con daño par, encadenar da lo mismo que el daño normal — y no por casualidad", () => {
    const r = applyDamageModifiers(24, "NECROTIC", [
      { damageType: "NECROTIC", effect: "RESIST" },
      { damageType: "NECROTIC", effect: "VULNERABLE" },
    ]);
    expect(r.total).toBe(24);
