...
| `espacios` | **La pasada de medición del pulido de 2026-09-12** (anexo #17, y cierra la medida de #6 y #8), sobre `HUECO_MAX_PX`/`DESNIVEL_MAX_PX` de la nota de diseño (§7): en las cuatro pestañas de la hoja a página (Números, Rasgos, Recursos, Estado) a 1280px, **`medirHermanas` mide TODAS las tarjetas de la pestaña, por columna** — no solo los hijos directos de la rejilla — para que una tarjeta apilada dentro de una sub-rejilla (Ficha + Personalidad, en Rasgos) no quede invisible a la medida; ninguna pareja de tarjetas de la misma columna deja un hueco vertical de más de 48px, y ninguna fila de tarjetas sin nada debajo en su columna (la exención que evita el falso desnivel de Salvaciones/Percepción pasiva, cuya columna se estira entera) se desnivela más de 24px. Ronda de arreglo 2 (2026-09-12): la primera versión medía `:scope > *` y la mutación del controlador (`gap-[10rem]` bajo «Ficha», 160px de hueco real) seguía pasando — el hueco vivía dentro de un hijo directo, no entre ellos. El panel de detalle de Objetos (`aside "detalle del objeto"`, `lg:sticky`) se pega justo bajo la banda fija tras desplazar, nunca por debajo con solape ni por encima; y que escribir una expresión inválida no mueve ni un píxel la tarjeta de tirar — ni en la pantalla «Dados» ni en el cajón angosto «La mesa tira», donde una pista larga tiene más motivo para envolver — porque `Field reservaEspacio` ya reservaba esa línea antes de que apareciera el error. **Lo que no cubre**: no repite las pruebas de contraste ni de afordancia de `hoja.spec.ts`, y no mide las pestañas Objetos, Ataques ni Conjuros, que no son rejillas de `grid-cols` (son listas de una columna sin el defecto que este anexo persigue). |
