...
| `tablero-en-la-mesa` | **C1 bis (2026-09-12, D-CF-63): el tablero PlanarAlly enmarcado en el centro de la mesa.** Con `boardRoomUrl` guardada, el `<iframe>` ocupa el centro **sin scroll de página** —la misma medida de `mesa-mide`, con el tablero puesto—, y el registro pasa a un cajón inferior: plegarlo y sellar nuevas anotaciones sube su contador, y desplegarlo lo vuelve a cero y enseña el hilo de siempre. La «sala» de la prueba es la propia `/acerca-de` (mismo origen: la web no manda `X-Frame-Options` contra sí misma), así que no sale a internet. Una segunda prueba deja escrito, con su cifra (`D-CF-26`), que a 390 px el marco y el cajón se apilan uno debajo del otro — la mesa a ese ancho sigue aplazada, y esto no lo resuelve. |
