...
| E-IB-18 | **`cancel()` no escribe suceso**: decisión del autor de que un combate que nunca empezó «no es historia, es un clic deshecho» — escribir uno sobre un `Encounter` recién borrado sería justo la historia que se decidió no guardar (R22) |
| E-IB-19 | **`setSide` y el reajuste de `activePosition` no emiten suceso todavía**: se decide con la pantalla, en las tareas 8 y 10, no a ciegas (R23) — sigue sin decidirse, ficha «P2-eventos» en `06-pendientes.md` |
