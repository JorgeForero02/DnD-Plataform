...
| `sessions/tablero/` | C1 bis (2026-09-12): el tablero externo enmarcado en el centro (`MarcoDelTablero`, con `CabeceraDelMarco` y su «abrir aparte»), cuando la campaña tiene `boardRoomUrl` (D-CF-63) y la banda está en «Con tablero». **El cajón inferior del registro (`CajonDelRegistro`) ya no existe** (Task 5 de 3A.3): el registro vive en `sessions/registro/` (`ColumnaDelRegistro`), columna lateral de 18rem con los filtros inline en su cabecera (D-CF-148). El tablero oficial es **Just Another VTT** (`Mine/mini-vtt`, del autor; decisión del 2026-09-18, sustituye a PlanarAlly, D-CF-57) |
| `sessions/registro/` | `ColumnaDelRegistro`: envoltorio de una línea que lleva el `useState` del filtro (Todo · Relato · Números) y monta `HiloDeSesion` con él; sin `boardRoomUrl` ocupa el centro, con él la lateral |
| `apps/web/src/features/sessions/objetivo.store.ts` | **A quién apunta la barra** (Task 4 de 3A.3): un `Objetivo` global a la pestaña, puesto desde `BotonDeApuntar` (tarjetas del elenco) o el chip de la franja, leído por `BarraDeAcciones` y `LanzarConjuro`; `useLimpiarObjetivoDeLaMesa` lo quita al terminar el combate o cambiar de campaña |
| `features/actions/` | **La barra de acciones** (Task 4 de 3A.3): `hooks.ts` (`useAcciones`, sondeo `SONDEO_DE_MESA_MS`) y `BarraDeAcciones.tsx` —cinco menús desde `AccionesResponse`, un `PanelFlotante` por grupo, y un control por fila que reutiliza la puerta de siempre: `useResolveAttack`/`useRollAttack` (Ataques), `LanzarConjuro` (Conjuros), `useUsarActividad` con la clave desnuda (Aptitudes y Básicas), `useConsumeInventoryItem` (Objetos), `AyudarA` (Ayudar). Cada control pinta el error del servidor en línea (`ErrorDeControl`). Los rótulos y frases salen de `dominio/acciones.ts` |
