...
>
> - **Ficheros de e2e de API:** 66 (`apps/api/test/*.e2e-spec.ts`), contados del disco.
> - **Ficheros de e2e de navegador:** 57 (`apps/web/e2e/*.spec.ts`), contados del disco.
>
...
**El mundo del taller, desde la Task 14 bis (2026-09-13, D-CF-64), se prueba por piezas** en
`features/sessions/taller/mundo/__tests__/`: `arbolDelMundo.test.ts` (el módulo puro, en `node`:
jerarquía, lateral que no mueve nada, dos padres, ciclo que termina, `sinHilos`, vecinos leídos
desde la ficha abierta), `DetalleDeFicha.test.tsx` (anillo con N botones y su frase, hilos sin
enums, «Añadir hilo» manda `{ toId, label }`, rechazo del servidor en línea, cambiar = crear antes
de quitar) y `ElMundo.test.tsx` (elegir pinta el detalle, el chip «Sin hilos», el buscador, el
teclado del `tree`). **Y dos suites se retiraron en ese mismo commit, por D4** (2026-09-02: «el
tablero telaraña se retira»): `TableroTelarana.test.tsx` y `posiciones.test.ts`, con sus dos
módulos. Es la única baja de pruebas de la tanda del pulido y está declarada, no escondida.

**La barra de acciones (Task 4 de 3A.3, T22), en `features/actions/__tests__/BarraDeAcciones.test.tsx`**:
los cinco contadores desde `AccionesResponse` (mocks de `actionsApi.fetchAcciones`, y
`spellbookApi.fetchSpellbook` para la fila de Conjuros, que resuelve contra la `SpellbookEntry`
real); una fila apagada con `aria-disabled` y su motivo en `aria-describedby`, traducido —nunca
la clave del servidor, ni el coste crudo, en el DOM—; con un objetivo sembrado en
`objetivo.store.ts` antes de montar, la fila de un conjuro de un solo objetivo ofrece «Lanzar
sobre &lt;nombre&gt;» sin lista que elegir; y pulsar «Usar» sobre una básica dispara
`usarActividad(…, "basic:dodge", …)`, la misma puerta que ya usa `Actividades.tsx`.
...
- **La accesibilidad está a medias y el rendimiento no existe.** Playwright **sí** mide
  contraste en los tres temas, **sí** cubre un caso de responsive real (que un control de
  formulario no dispare el zoom de iOS Safari) y **sí** tiene un recorrido de teclado (`teclado`,
  arriba: `Tab`/flechas/`Enter`/`Escape`, foco y su anillo visible, sobre la navegación y —desde
  la tarea 8 del pulido— el menú «…» del elenco). Falta el resto: lectores de pantalla de
  verdad, anchos intermedios, y cualquier medida de rendimiento.
...
| `characters` | Quien crea un personaje queda como su dueño **según el servidor, no según lo que mande el cliente**; el listado del jugador se filtra por `canView` y el DM los ve todos; **editar exige ser dueño o DM, y otro jugador recibe 403**. Es la puerta de entrada a la hoja, y llevaba sin aparecer en este mapa desde que se escribió. **Archivar (2.5.8, ficha M9)**: saca al personaje del listado sin borrar nada —contando filas de verdad, no fiándose del 200—, recupera hoja/inventario/dinero enteros, deja su rastro en la línea de tiempo, y solo dueño o DM pueden archivar. **D-CF-66: `level` es DM-only** — el dueño recibe 403 al mandarlo en el `PATCH`, y el resto de sus campos (`bio`, por ejemplo) le sigue funcionando igual. |
| `reglas-de-la-mesa` | **La tabla de seguridad de la spec (§7), contra la base real**: solo el DM cambia `tableRules` —403 para el jugador—, y `GET` la devuelve siempre con los defaults rellenos aunque la campaña no la tenga puesta. `POST /characters` **ignora** el nivel del cuerpo: el personaje nace a `nivelInicial` pase lo que pase (E-RM-1). Con `MATRIZ`/`PUNTOS`/`DADOS` el `PATCH .../sheet` exige **las seis características a la vez** —cinco de seis, una repetida o un reparto que no es la matriz/el presupuesto son 400 con el porqué—; una raza, clase o subclase fuera de `permitidos` es 400 con su nombre legible aunque el cliente la mande, y la lista vacía deja pasar cualquiera. **«DADOS 4d6kh3 con 2 intentos»** es el recorrido entero de un intento: dos tiradas devuelven seis valores entre 3 y 18 cada una, el tercer intento es **409** (sin tirar nada, contado sobre las filas de `AbilityRollAttempt`), unos valores que no salieron de ningún intento del personaje son 400, fijar con el segundo intento marca `chosen`, y **después de fijadas, un `PATCH { abilities }` es 400 para el dueño con o sin `attemptId`** —tampoco puede volver al primer intento mandando su id: la lista sigue `[false, true]` y la hoja intacta (I-1, ola de arreglos 1)—; el DM sí corrige las seis juntas sin `attemptId`, y el dueño sigue con 400 después (E-RM-13). Y a nivel 3 con `TIRADA` y `ORO_TABLA`: al fijar la clase, `hitPointsPerLevel` guarda sus dos valores, la bolsa recibe el oro de la tabla y `maxHp` de la hoja ya los refleja, con la traza señalando `maxHp.perLevelAtCreation`. |
...
| `level-up` | Subir de nivel escribe su suceso y sube los PG máximos en la cantidad prevista; el previo es idempotente; **el nivel 20 es el techo**; y desde D-CF-66, **el dueño recibe 403 en el previo y al confirmar — solo el DM sube de nivel**. |
...
| `dano-extra` | 3A.2 (Task 8, D-CF-129) — **el daño extra al impactar, marcado por el jugador y confirmado por el DM.** Un pícaro nivel 3 con una daga y un paladín nivel 3 con una espada larga impactan de sobra sobre sendos goblins (CA anulada a 1); sobre la tirada `DAMAGE` con `pendingDamage`, el pícaro marca `sneak-attack` — 201, `label: "Ataque furtivo (2d6)"` (tabla de escala del SRD para nivel 3), sin gastar ningún recurso — y el paladín marca `divine-smite` — 201, `"Castigo divino (2d8)"`, gasta `spell-slot-1` (nivel 3: solo hay espacios de nivel 1) con su propio `RESOURCE_SPENT`. **El dueño del atacante deja de ver 404 en `damage-preview`**: ahora ve `{ extrasDisponibles, extras }`, sin `target` ni `resulting` ni el nombre del goblin — comprobado con `JSON.stringify` sobre el cuerpo entero, no solo con una aserción de propiedad. Marcar la MISMA clave una segunda vez es 409; un guerrero sin el rasgo es 400 con su nombre (`"Thorin el guerrero no tiene Ataque furtivo."`); quien no es dueño del atacante ni DM es 403. El DM aplica y el `HP_CHANGED.delta` es la suma del daño base y el extra, releída del preview justo antes de aplicar. **Ola de arreglos (I-2/I-3)**: `apply-damage` y `damage-extra` lanzados a la vez en `Promise.all` sobre la misma tirada — si el extra respondió 2xx, el `HP_CHANGED` incluye su cantidad; si respondió 409, el espacio del paladín sigue intacto (la transacción entera se deshizo). |
| `encantar` | 3A.2 (Task 9, T15, D-CF-130) — **Arma mágica como `TemporaryModifier` sobre un objeto.** Un mago nivel 3 con `magic-weapon` PREPARADO encanta la espada larga EQUIPADA de un guerrero — otro jugador, «aliado visible» —: 201, gasta `spell-slot-2` (nivel base del conjuro), y `ACTIVITY_USED` trae `name: "Arma mágica"`, `spellLevel: 2`. El cuadro de ataques del guerrero sube **+1**, con un paso de traza propio (`sourceType: "temporary"`, `labelKey` con «Arma mágica») — no confundido con el `+N` de un objeto mágico permanente. La concentración queda en **quien lanza**, no en quien lleva el arma (`concentrating-magic-weapon`). Sin `itemId` es 400; sobre un objeto que no existe, 404. Avanzar el reloj 3601 s vence el encantamiento (el ataque vuelve al bono base) y la condición de concentración a la vez — la misma hora. |
| `acciones` | 3A.3 (Task 1, T21) — **la barra de acciones, `GET .../actions`, contra un combate real.** Un mago nivel 3 con `magic-missile` PREPARADO y un goblin, en un encuentro `ACTIVE`: en su turno, `magic-missile` sale disponible con `motivos: []` en `grupos.CONJUROS`, y las ocho básicas siempre están en `grupos.BASICAS` (`basic:dodge` incluida). Tras usarlo por `usar()`, la siguiente `GET` marca `motivos: ["ACCION_GASTADA"]` sin apagar el resto del turno (`esMiTurno` sigue `true`). El DM avanza el turno y la misma acción, y una básica cualquiera, pasan a `["NO_ES_TU_TURNO"]`. Un jugador que no es dueño ni DM del mago recibe **403** — no una versión redactada. **Y desde la ola post-revisión de 3A.3 (C1): el contrato de la barra para una APTITUD** — la clave tal cual la lista `GET actions` (`feature:arcane-recovery`, `disponible: true`) se reenvía a `POST …/activities/<clave>/use` → 201, y la siguiente `GET` la marca `SIN_USOS`; comprobado por mutación (404 sin `parsearClaveDeActividad` aceptando `feature:`). |
...
| `pnj-del-mundo` | **PNJ del mundo y la mesa (spec 2026-09-14), contra Postgres real.** El puente `entityId` (Task 0): un cuerpo enlazado a su ficha del mundo se redacta a `null` para quien no puede ver la ficha, en las seis lecturas; validado (400 fuera de campaña o si no es NPC). **Revelar y ocultar** (Task 1/1 bis): antes de `reveal` el jugador no lista la criatura y su ficha del mundo le da 404; `reveal` es 403 al jugador y 201 al DM con las tres columnas subidas —instancia, ficha del mundo y plantilla, si estaban por debajo—; el jugador ve entonces `statblockRef` y `entityId`; el registro trae un `NPC_REVEALED` con el nombre de la ficha; `hide` baja **solo** la instancia; **un segundo `reveal` no cambia nada y no escribe ningún suceso** (idempotente); y subir la ficha del mundo a mano (`PATCH /entities/:id`) sube también a los cuerpos vivos enlazados. **Sacar del combate** (Task 2): `DELETE …/combatants/:id`, contra el encuentro `ACTIVE` y nunca al último combatiente. |
...
| `ataque-de-conjuro` | 3A.2 (Task 5, T19) — **el ataque de conjuro contra la CA, la MISMA mecánica que un arma** (`resolverAtaqueContraCa`, extraída de `resolveAttack`). Un mago lanza `fire-bolt` contra un goblin cuya CA el DM anula a 1 (`PUT .../overrides/ac`, como `pnj-en-la-mesa.e2e-spec.ts`) para dejar el veredicto determinista pese al 20/1 natural: `HIT`/`CRITICAL` deja un `ATTACK_RESOLVED` y un `ABILITY_ROLL.pendingDamage` con `reason: "Conjuro: Descarga de fuego"` y `attackResolvedEventId` puesto — la CA no aparece en ninguno de los dos sucesos — y el DM lo aplica (los PG del goblin bajan). Con la CA anulada a 30, `MISS` y ninguna tirada de daño en esa respuesta. Sin objetivo, solo la traza del bono, como antes de esta tarea; con más de un objetivo, 400 («un ataque tiene un objetivo»); contra uno mismo, 400. |
...
| `dano-con-su-traza` | El daño aplicado desde la tirada (2.5.4). **`rollEventId` sobrevive el viaje de ida y vuelta por la columna `Json`** y aparece en el `HP_CHANGED` del registro; uno inventado o de otra campaña es 400 contra la base real, no contra un mock que siempre dice que sí. Un personaje **de nivel 8** con una condición `concentrating-*` que recibe 25 de daño hace que el sistema **pida** una salvación de Constitución con CD 12 — el nivel es parte de la prueba: a nivel 1 esos 25 son muerte masiva y no se pide nada, que es lo que dejaba este recorrido en rojo el día que se escribió sin ejecutarlo, visible donde `GET /roll-requests` la sondea de verdad — y responderla sigue el camino de 2C.5 sin tocar. Dos golpes concentrados piden **dos** peticiones, nunca una. **Desde la tarea 11 del pulido (C4, #15)**: el DM cita `sourceCharacterId` a mano y el `HP_CHANGED` lo lleva; un origen que no existe en la campaña es 404 (`requireVisibleCharacter`) y **no se escribe nada**, comprobado leyendo los PG antes y después. **Desde el cierre del 2026-09-14 (tarea 11)**: un origen que SÍ existe en la campaña pero que el actor no ve —un PNJ `DM_ONLY` citado por un jugador ajeno, sobre su propio personaje— es el mismo 404, sin `HP_CHANGED` nuevo, distinguiéndolo del caso «no existe» de arriba. |
| `concurrencia-puerta` | **Concurrencia real contra Postgres (cierre, 2026-09-14) — lo que las unitarias no pueden medir porque el Prisma simulado no bloquea filas.** Dos `POST …/apply-damage` sobre el mismo `pendingDamage` en `Promise.all`: **exactamente** una 2xx (la que gana el `WHERE … IS NULL`) y una 409, y **un solo** `HP_CHANGED` con ese `rollEventId` en el registro. Dos `POST /xp` iguales en `Promise.all` sobre el mismo personaje: **las dos** 2xx, y `xp.actual` termina en la **suma** de las dos, no en una pisando a la otra — `XpService.award` bloquea la fila (`FOR UPDATE` dentro del mismo `UPDATE`) antes de sumar. |
...
| `puerta-de-efectos` | **La puerta de efectos (spec 2026-09-12, §3/§4 bis/§5/§5 bis), contra Postgres real — cinco bloques.** El describe raíz (**la segunda puerta y el daño de una salvación, tarea 2**): `cura-de-prueba` (sin salvación) comprueba que la puerta nueva no rompió el camino directo que ya pasaba por `changeHpFromEffect`; `bola-de-prueba` (SRD *Fireball*, CD 15, `8d6` de fuego, mitad si salva) se tira **una sola vez** en `usar()` y las DOS peticiones que salen de ahí cargan el **mismo** `pendingEffect.amount`; fallar la salvación quita el total, empatar la CD quita la mitad redondeada abajo; A no puede pedir una tirada directamente ni tocar los PG de B a mano (403, sin cambios); y usar una actividad sobre un PNJ `DM_ONLY` es 404 **sin gastar el recurso**. **`la bandeja de daño` (tarea 3, §4b.4-§4b.7)**: A equipa un arma y el DM sube un `SRD:wight` (resistente a cortante); resolver el ataque y tirar el daño citándolo deja el `ABILITY_ROLL` con `pendingDamage` apuntando al wight; el preview es **200 para A, pero solo con `extrasDisponibles`/`extras` (Task 8, 3A.2: A no tiene ningún rasgo de daño extra, así que ambos salen vacíos, y sin `resulting` ni nombre del objetivo)** y 200 completo para el DM, con la reducción por resistencia ya aplicada; aplicar es 403 para A y 200 para el DM, que firma el `HP_CHANGED` con el nombre de A, y **un segundo clic del DM es 409**; un daño sin `attackRollEventId` no lleva `pendingDamage`, así que su preview es 404 hasta para el DM. **`hasta el próximo descanso`**: aplicar `poisoned` hasta un descanso corto y `frightened` hasta uno largo, y mandar `durationSeconds` **y** `expiresOnRest` a la vez es 400 (son excluyentes). **`XP` (tarea 5, §5 bis)**: activar el modo XP en las reglas de la mesa; un jugador no puede dar XP (403); dárselo a un PNJ de statblock es 400; dar 450 PX dos veces dejan `xp.actual === 450` en la hoja de A; en modo `HITO` la hoja **no trae** `xp`; y terminar un combate de dos goblins contra A y B **propone** `xpPropuesto.total 100, porCabeza 50` sin repartir nada hasta que el DM lo pida — terminar sigue siendo 403 para un jugador, regla que ya existía. |
...
| `desbordes` | **D-CF-67 — «nada se sale».** Con la semilla demo (`scripts/seed-demo.mjs`, corrida en `beforeAll`, entrando como la DM), mide en Chromium real lo que `jsdom` no puede: (1) un texto que se sale de su propia caja visible (`scrollWidth > clientWidth` con `overflow: visible`, solo en hojas de texto), y (2) un elemento fuera del rectángulo de su ancestro recortador más cercano o de la ventana — excusando el desplazamiento legítimo, **salvo en desplegables abiertos, que tienen que caber en el cliente del ancestro sin hacerle scroll**. Ignora `<select>` nativos, `.sr-only`, `aria-hidden` y el `iframe` del tablero; la comprobación contra la ventana es solo en X (una página que se desplaza en vertical no es un desborde). **Tres casos, los que dio la auditoría** (recorte del autor: no hay recorrido genérico): la lista de objetivos al atacar, el menú «…» de la fila más baja del elenco, y la traza de «Comp.»; cada uno deja su captura en `apps/web/e2e-resultados/desborde-*.png` — el reconocimiento visual que el autor mira. **La corre el propio implementador** de esa tanda (solo este fichero), excepción declarada en D-CF-67. |
| `reglas-de-la-mesa` | **D-CF-53/65, con dos navegadores.** El DM fija «Con dados, 3d6, 2 intentos» en «Reglas de la mesa» y **sobrevive a una recarga entera**, que es lo único que demuestra que llegó al servidor; invita, y el jugador crea su personaje **sin ver ningún spinbutton «Nivel»** —solo la frase «lo fija la mesa»—. En la hoja, las seis casillas ya están apagadas con el motivo antes de tirar nada; «Tirar características» trae **seis `role="status"`, cada uno con tres `[data-dado]` y tres `[data-icono="d6"]`** —lo que ninguna prueba de API puede enseñar es la CARA del dado, no solo su total—; con los dos intentos agotados **el botón no desaparece ni se deshabilita** (regla de la casa): un tercer clic escribe «Ya usaste los 2 intentos.» en línea y no aparece ningún «Intento 3 de 2»; al elegir el segundo intento con los seis selects (a cada uno el primer valor que quede: se agotan) aparece «Fijadas con dados», la casilla de Fuerza deja de estar vacía y **las seis casillas siguen deshabilitadas** mientras la hoja deriva («Salvaciones» visible). La segunda prueba es la otra mitad: una clase fuera de `permitidos` no se ofrece al crear un personaje nuevo, y la clase ya guardada de uno existente que dejó de estar permitida **se ve marcada y deshabilitada en su propia opción**, nunca oculta. **No se corre en esta ficha**: como el resto de la tanda, la ejecuta el orquestador al cerrar (D-CF-65, ver más abajo). |
| `puerta-de-efectos` | **Tarea 9 de la puerta de efectos (spec 2026-09-12), tres recorridos con dos navegadores, cada uno con un `expect` que mira el navegador del OTRO contexto.** (2) La condición «hasta el próximo descanso largo»: el DM se la aplica al personaje de B con el radio nuevo (`Condiciones.tsx`) y **la hoja de B ya lo dice sin que B haya tocado nada**; un descanso corto no la toca, uno largo la retira, y el hilo de la sesión traduce `REST_DECLARED` como «Descanso largo» **y** dice por qué se fue la condición: «Se le quita la condición «Asustado» — Descanso largo» (spec §5.3). (3) La bandeja de daño (`BandejaDeDano.tsx`): con la CA de un goblin anulada a 1 desde «Anulaciones del DM» (para no depender de una tirada de ataque concreta), A lo ataca y tira el daño desde su cuadro de ataques; en el hilo del DM la tarjeta trae «Aplicar el daño a Goblin» y en el de A solo «Daño pendiente» —el mismo 404 que un objetivo que no se ve—; al aplicar, **los dos** ven «Aplicado» sin recargar —el DM porque `useApplyDamage` relee el preview, y A **no por el preview, que para él sigue siendo 404**, sino por `pendingDamage.appliedEventId`, el candado que viaja en el propio suceso del hilo y que el canal en vivo le trae (ola de arreglos 1)— y los PG del goblin bajan en el elenco (`role="img"` con su `aria-label`). (4) El XP: con la regla de la mesa en «Por experiencia», la hoja de A dice «0 / 300 PX»; el DM da 300 desde «Dar XP» y **el marcador de A sube a «300 / 300 PX» con el aviso de nivel 2 sin que A haya pedido nada**, y el nivel de A, comprobado contra la API, se queda en 1. **El recorrido (1) del brief —un clérigo cura a un guerrero con una actividad de salvación— se omite**: `Actividades.tsx`, la única pantalla que usa una actividad del catálogo, manda `usar.mutate({ activityKey })` sin ningún campo `objetivos` — no hay selector de objetivo en esa pantalla, así que ninguna actividad con salvación se puede dirigir a otro personaje desde el navegador hoy, sea lo que sea que exponga el catálogo. Ese camino lo cubre `apps/api/test/puerta-de-efectos.e2e-spec.ts` contra la API real. **No se corre en esta ficha** (D-CF-65): la ejecuta el orquestador al cerrar. |
...
| `teclado` | **El recorrido entero solo con teclado** (Task 32, U6): de «Tus crónicas» a una campaña, a «El mundo», a la ficha de un PNJ y de vuelta, sin tocar el ratón. En cada parada se comprueba `document.activeElement` **y** que el anillo de foco calculado no sea `outline-style: none` (WCAG 2.4.7). `ArrowRight` entra porque la tira de pestañas es un `tablist` con «roving tabindex» (patrón WAI-ARIA): solo la pestaña activa lleva `Tab`, y ningún `tabindex="-1"` huérfano se cuela salvo el de las pestañas inactivas. **Desde la tarea 8 del pulido (C2: #1)**, un segundo recorrido abre el menú «…» de un combatiente: `Tab` hasta «Más acciones sobre …», `Enter` lo abre y mueve el foco a su primer ítem, `ArrowDown` lo pasa al siguiente, `Escape` lo cierra sin seleccionar y devuelve el foco al botón, y un segundo `Enter`+`Enter` selecciona «Condición» de verdad y abre su cajón. |
...
| `conjuros` | **3A.2 (Task 6, T11) — elegir: la pestaña «Conjuros», con un mago hecho por la API** (misma técnica que `combate.spec.ts`/`hoja-pestanas.spec.ts`, para no recorrer el formulario). **Arreglo previo a la Task 7 (paso 0a)**: el mago sube a nivel 3 con un segundo `PATCH` del DM — a nivel 1 el tope de preparados real (`topeDePreparados` = mod + nivel = 3+1 = 4) no coincidía con el «de 6» que esta prueba afirmaba, un número que en realidad era el tamaño del libro; a nivel 3 el tope de preparados es 3+3=6, así que el texto no cambia y el que sí cambia es el libro, que ahora cabe hasta 10. El mago nace con su libro sembrado (Task 3, D-CF-125): «6 de 10 en el libro» en la cabecera de «Listos para lanzar» antes de tocar nada. Preparar «Proyectil mágico» y «Escudo» desde «Disponibles» sube el contador a «2 de 6 preparados» **y las mueve de zona** —dejan de estar en «Disponibles», aparecen en «Listos para lanzar»—; conocer el truco «Descarga de fuego» sube «1 de 3 trucos» sin tocar el de preparados. Buscar «bola» en «Disponibles» deja **dos** filas («Bola de fuego» y «Bola de fuego de explosión retardada», que también pasa el filtro), la nuestra entre ellas, marcada **«Fuera del libro»** con el botón «Añadir al libro» — nunca desaparecida, ni deshabilitada por ningún tope. **Paso 0b**: a 1280×800 la altura de una fila de «Disponibles» es ≤ **56** px (una línea ronda los 50, dos los 78 — el umbral separa las dos medidas reales, `FilaDeConjuro.tsx`); a 390 sí envuelve. **Capturas** a 1280×800 y 390×844 en `apps/web/e2e-resultados/conjuros-{1280,390}.png`. Escrita por el implementador y **corrida en verde por el orquestador el 2026-09-18** (D-CF-65: quien la escribe no la corre). |
| `lanzar` | **3A.2 (Task 7, T11 parte 2) — lanzar: el botón «Lanzar» de la pestaña Conjuros, con dos navegadores (DM y una maga hecha por la API, nivel 3, mismo atajo que `conjuros.spec.ts`) y un goblin instanciado y revelado por la API con la CA anulada a 1 por la UI de «Anulaciones» (mismo patrón que `puerta-de-efectos.spec.ts`).** La maga abre «Tu hoja» desde la mesa (patrón de `furia.spec.ts`), pestaña Conjuros, y lanza «Proyectil mágico» (`objetivos: "varios"`, mecánica `dados`): elige el espacio de nivel 1 y marca «Goblin», «Lanzar sobre 1» — en su hilo aparece la línea de «lanza Proyectil mágico» y una tarjeta «Daño pendiente»; la tarjeta de «Espacios de conjuro» pasa a «Nivel 1: 3 / 4» sin recargar (`useUsarActividad` invalida `spellbookKey` y el hilo además de recursos/condiciones/hoja/encuentro — fix round 3 de la ola: en el gancho, no en el `onSuccess` del `mutate`, que no corre si el componente se desmonta antes de la respuesta); **en el navegador del DM** la misma tarjeta trae «Aplicar el daño a Goblin» y no en el de la maga — al aplicarla, los PG del goblin bajan en «En la mesa». Después la maga lanza «Descarga de fuego» (`objetivos: "uno"`, mecánica `ataque`): un solo clic sobre «Goblin» elige Y lanza a la vez, sin lista de casillas ni botón de envío — con la CA anulada a 1 se reintenta hasta impactar (tope de diez, mismo margen que `puerta-de-efectos.spec.ts`), y el veredicto (`ATTACK_RESOLVED`, `linea-de-log.ts`) y otra tarjeta pendiente aparecen en el hilo. Escrita por el implementador y **corrida en verde por el orquestador el 2026-09-18** (D-CF-65). |
| `barra-de-acciones` | **3A.3 (Task 4, T22) — la barra bajo el marco, con dos navegadores (DM y una maga nivel 3, mismo atajo de fixture que `lanzar.spec.ts`) y un goblin con la CA y los PG máximos anulados.** Avanza el turno (bucle, igual que `acciones.e2e-spec.ts` de la Task 1: la iniciativa no está fijada) hasta que le toca a la maga; apunta al Goblin desde su tarjeta en «En la mesa» (`role="button"`, `aria-pressed`) y el chip de la barra dice «apuntas a Goblin»; abre «Conjuros», «Lanzar» Proyectil mágico y, con el chip puesto, un único «Lanzar sobre Goblin» confirma sin pasar por la lista de objetivos — en el hilo aparece «lanza Proyectil mágico», en el DM «Aplicar el daño a Goblin», y la franja `status "Economía del turno"` pasa a «acción: gastada». La misma fila, que se queda abierta, se apaga sola (`aria-disabled`, sin volver a pulsar «Conjuros») con «ya gastaste tu acción» — el motivo traducido del servidor, no su clave. El DM pasa turno dos veces y vuelve a ser el de la maga; «Esquivar, ayudar…» → «Esquivar» deja «usa Esquivar» en el hilo. Escrita por el implementador, **no corrida en esta ficha** (D-CF-65): la ejecuta el orquestador. |
| `mesa-prototipo` | **3A.3 (Task 6, D-CF-149) — la mesa medida contra el HTML del prototipo, con capturas.** Tres navegadores (DM, Sylas mago nivel 3, Corvin guerrero nivel 3), un goblin revelado, sesión en curso y encuentro `ACTIVE` con los tres (cada jugador tira su iniciativa); la «sala del tablero» es `public/tablero-de-prueba.html`, servida por Vite en el mismo origen. Saca **ocho capturas** a `apps/web/e2e-resultados/` (se commitean: son el reconocimiento visual para el autor): `mesa-{dm,jugador}-{1280,390}.png` en modo «Con tablero», `mesa-{dm,jugador}-cronica-1280.png` en «Sin tablero», y `mesa-prototipo-{1280,390}.png` del propio HTML abierto por `file://`. Y **seis medidas**: (1) a 1280 el `main` tiene tres pistas de rejilla para el DM y dos para el jugador en crónica, sin scroll de página en ninguno; (2) una sola `banner "Estado de la mesa"` con el título de la sesión y el reloj, y ninguna `region "La escena"`; (3) `region "Orden de turnos"` entre la banda y el `main`, con `status "Economía del turno"` y ningún botón «Usar mi…»; (4) la barra de acciones debajo del marco, alineada con él, con sus cinco botones visibles; (5) en la lateral del DM el registro encima de `complementary "Herramientas del DM"`; (6) ninguna `region "Registro en vivo"`. La única prueba de navegador que el implementador de la Task 5b corrió él mismo (excepción declarada de D-CF-67, tarea visual): captura → comparar con el prototipo → corregir → capturar. |
| `espacios` | **La pasada de medición del pulido de 2026-09-12** (anexo #17, y cierra la medida de #6 y #8), tres rondas, sobre `HUECO_MAX_PX`/`DESNIVEL_MAX_PX` de la nota de diseño (§7). `medirHermanas` mide TODAS las tarjetas de la pestaña (`section[aria-label]` y `[data-tarjeta]`), por columna — no solo los hijos directos de la rejilla — para que una tarjeta apilada dentro de una sub-rejilla (Ficha + Personalidad, en Rasgos) no quede invisible a la medida: la ronda 1 medía `:scope > *` y la mutación del controlador (`gap-[10rem]` bajo «Ficha», 160px de hueco real) seguía pasando. Corre a 1280px sobre Números, Rasgos, Recursos, Estado y **Ataques** (ronda 3: misma forma `lg:grid-cols-2 items-start` de dos tarjetas). Ninguna pareja de tarjetas de la misma columna deja un hueco vertical de más de 48px. **El desnivel exime a toda tarjeta con otra tarjeta debajo en su misma columna** (Salvaciones en Números, que desde la ronda 3 crece con `flex-1` para repartirse el alto sobrante con Percepción pasiva en vez de que solo el envoltorio invisible se estirara) y compara la última tarjeta de cada columna agrupada por techo compartido; ninguna fila se descarta (la ronda 2 traía un descarte de la fila más baja que nunca se ejecutaba con el contenido real, y se quitó en la ronda 3). **Cláusula honesta**: con el contenido de hoy (un guerrero de nivel 1, sin conjuros) el desnivel en Rasgos, Recursos y Estado casi nunca tiene una fila con 2+ tarjetas comparables que medir — sus columnas se apilan a profundidades distintas y sus techos no coinciden —, así que en la práctica el desnivel de esas tres pestañas queda sin ejercitar por esta suite; Números (y ahora Ataques) son las que de verdad prueban la fórmula. El panel de detalle de Objetos (`aside "detalle del objeto"`, `lg:sticky`) se pega justo bajo la banda fija tras desplazar, con cota de arriba y de abajo desde la ronda 3 (antes solo el «no por debajo»; sin la cota de arriba un `--banda-fija-alto` roto que reportara 0 habría pasado con el detalle a 16px de la banda), leyendo `--space-4` resuelto a píxeles por el navegador y no por el texto crudo del token (`1rem`, no `16px`). Y que escribir una expresión inválida no mueve ni un píxel la tarjeta de tirar — ni en la pantalla «Dados» ni en el cajón angosto «La mesa tira», donde una pista larga tiene más motivo para envolver — porque `Field reservaEspacio` ya reservaba esa línea antes de que apareciera el error. **Desde la Task 10**, «Qué se tira» vive bajo «Modo avanzado» (y, en el cajón, la guía del SRD bajo «Audiencia y CD»): las dos medidas abren esos `<details>` antes de tomar la medida «antes». **Y desde la tarea 8 del pulido (C2: #1)**: la fila de mandos de una tarjeta del elenco —«Daño», «Curar» y el botón del menú «…»— no se sale del rectángulo de su propia tarjeta, a 1280×800 (`boundingBox` de la fila dentro del de la tarjeta, ±1px). **Lo que no cubre**: no repite las pruebas de contraste ni de afordancia de `hoja.spec.ts`; no mide Objetos (tiene su propia rejilla de inventario con panel sticky, medida por la prueba del anexo #6 de esta misma suite) ni Conjuros (solo se monta para quien lanza, y el guerrero de esta suite no); y el desnivel de Rasgos/Recursos/Estado, por la cláusula honesta de arriba, no está realmente ejercitado con el contenido de estas pruebas. |
| `subir-nivel` | El servidor propone el diff, **tirar no aplica nada**, y confirmar deja la hoja en el nivel nuevo. |
| `dados` y `tirada` | El desglose y no solo el total; **el dado descartado pintado tachado** —que solo se puede medir en un navegador—; el motivo del evaluador real junto al campo; y **a ciegas, el total no viaja: se mide sobre la respuesta HTTP, no sobre el DOM**. **Desde la Task 10 del pulido, la bandeja de dados** (`BandejaDeDados.tsx`): pulsar d6, d6 y d20, subir el modificador, tirar sin escribir nada, y que el resultado enseñe los tres dados con su forma (`data-icono`); y en `tirada`, que el cajón compacto «La mesa tira» cabe en 17rem de ancho sin que la fila de siete dados ni la pila se salgan de él. |
...
| `tablas-del-dm` | Que lo primero que se lea sea **que esto no es del manual**; que el interruptor diga su posición **leída del servidor**; y que un error enseñe **la frase del servidor** y no una genérica. **Y desde la ficha P2-2, el eslabón que faltaba**: el DM redacta la entrega de una fila **desde la pantalla** —objeto del catálogo con su cantidad y monedas, en el panel propio de la fila—, guarda, tira, y la tirada entrega lo que sembró. El recorrido **no termina al crear**: edita el texto, guarda y vuelve a tirar, porque el defecto que la ficha destapó era que editar **borraba** la entrega. |
...
| `pnj-del-mundo-en-vivo` | **PNJ del mundo y la mesa (spec 2026-09-14), con dos navegadores.** Un Bandido nace `DM_ONLY` por la API (`POST /npcs`); entra en combate junto a los dos personajes; **de partida, la jugadora no lo ve en «Orden de turnos»** (lee «Alguien») **y el DM lo ve marcado «oculto»**. El DM lo revela desde «Más acciones sobre Bandido», en el elenco, y **sin recargar** la jugadora lo ve aparecer en el orden y «Bandido entra en escena» en su hilo. El DM lo saca del combate, y **en las dos pantallas** deja de estar en el orden, con «Bandido sale del combate» en el hilo de la jugadora — la mitad que ningún e2e de API puede demostrar: que el canal en vivo lleva el cambio de visibilidad hasta la otra pantalla sin que nadie la toque. |
...
| `tokens-contrast` | **El contraste real, medido, en los tres temas** (Oscuro, Claro y Lectura) y en las pantallas de sesión, campaña, 404, atribución y cuenta. Desde la Task 14 bis mide también **el mundo como árbol con detalle**: la raíz de tipo en cobre y su contador, la ficha del árbol y la elegida, el rótulo de jerarquía en gris, «Abrir ficha», un vecino del anillo y la fila de hilos. Desde el plan 05 mide también **las ocho voces de personaje**, cada una sobre `--bg` y sobre `--surface`, que son los dos fondos donde se pintan: **ninguna voz nueva se acepta sin su medición**, y los nombres salen del propio DOM para que un color añadido a `CHARACTER_COLORS` se mida solo. Y que un control de formulario **no dispare el zoom de iOS Safari** en un puntero basto. **Desde la tarea 8 del pulido (C2: #1)**, una superficie más: el menú «…» del elenco (`ui/MenuDeAcciones.tsx`) abierto de verdad —no en `/design-tokens`— sobre un combatiente real de la mesa, texto de un ítem y borde del panel flotante. **Desde la Task 10**: la bandeja de dados (`BandejaDeDados.tsx`) en la pantalla «Dados» — el botón de un dado, la pila una vez pulsado, el rótulo «Modo avanzado» y el borde del campo «Qué se tira» una vez abierto. |
...
| `mesa-en-estrecho` | **La mesa del DM a 390 px**, que es el ancho que `mesa-mide` no cubre —aquella mide 1280 y 1920, donde la rejilla va sobrada—. Trae **dos** pruebas y las dos importan: una **en verde** que deja escrito que a 390 px la página *no delata nada* (ni barra horizontal ni vertical) con las «Herramientas del DM» fuera de la ventana, y una con **`test.fail()`** que fija el defecto con su cifra: el borde derecho del panel en **550 px dentro de 390**, sus seis botones entre 426 y 537, y **el hilo estrangulado a 48 px de ancho**. Sus cinco aserciones son **blandas** para que todas se evalúen en cada pasada en vez de abortar en la primera: si no, las guardas contra el arreglo falso serían código sin estrenar. `test.fail()` **no es una prueba desactivada** —se ejecuta, y se pone roja el día que el defecto se arregle—, y **la prueba en verde también lleva una aserción que fija el defecto**, avisado en su mensaje. La ficha con las tres salidas está en [06-pendientes.md](./06-pendientes.md). |
...
| `mundo-arbol` | **Task 14 bis (2026-09-13, D-CF-64): el mundo como árbol con detalle, de punta a punta.** El DM en reposo, con un lugar y un PNJ montados por API: elegir el PNJ en el `tree` pinta su detalle; tender «vive en» hacia la Torre Gris desde los dos desplegables con buscador **mueve al PNJ bajo la Torre en el árbol**, con su rótulo, y el anillo lo enseña; **sobrevive a una recarga entera**, y bajo la raíz «PNJ» ya no queda ningún Corvin suelto. Una segunda prueba monta a 1280 y **solo entonces** estrecha a 390 px: el detalle va **debajo** del desglose (`boundingBox`: `detalle.y >= árbol.y + árbol.height - 1`) y `document.documentElement.scrollWidth <= 390`. Solo prueba que **las dos mitades de «El mundo» se apilan sin montarse** y que nada desborda en horizontal; la rejilla exterior del taller a 390 px no se apila (columna de ~180 px) y sigue aplazada (D-CF-26). La ronda 1 la cazó en rojo (detalle 64 px por encima del final del árbol, por `min-h-0` y scroll propio del árbol bajo `lg`). |
| `tablero-en-la-mesa` | **C1 bis (2026-09-12, D-CF-63) reescrita en la Task 5 de 3A.3: el tablero externo enmarcado en el centro de la mesa, con el registro como columna LATERAL (ya no hay cajón inferior) y con hilo LARGO (doce anotaciones, como `mesa-mide`).** Con `boardRoomUrl` guardada, a 1280×800: (1) la raíz `h-screen overflow-hidden` no scrollea; (2) **el marco ocupa al menos el 60 % del alto de `main`** —sin cajón que le dispute la fila, solo la barra de acciones debajo—; (3) el hilo scrollea **por dentro** de su panel (`scrollHeight > clientHeight` sobre «Sucesos de la sesión»); (4) el compositor «Qué anotar» sigue dentro de la ventana; (5) el registro cae a la DERECHA del marco (`x` del registro ≥ borde derecho del marco) y **su cabecera, con los filtros inline (D-CF-148), mide ≤ 48 px** —antes eran un `GrupoDeRadios` de ~200 px encima del panel. La «sala» de la prueba es la propia `/acerca-de` (mismo origen), así que no sale a internet. Una segunda prueba monta la mesa a 1280×800 y **solo entonces** reduce a 390 px: es **`test.fail`** con la medida buena (el registro APILADO debajo del marco) porque hoy las columnas fijas siguen lado a lado (D-CF-26, ola post-revisión de 3A.3, M9); se pondrá «pasó inesperadamente» el día que la mesa se apile. No demuestra que la columna central quepa o se lea a 390 px. |
| `dar-a-un-pnj` | **Que la segunda lista del selector de «Dar…» llega de verdad** (ficha P2-3): un Ogro bajado a la mesa desde el bestiario aparece entre los destinatarios, junto al personaje que ya estaba. La aserción del personaje va **antes** a propósito: sin ella, un cajón vacío por cualquier otro motivo se leería como «el PNJ no llegó». Comprobado por mutación: quitando la lista de PNJ del `useMemo`, el Ogro desaparece y el personaje sigue. El caso que la ficha nombra —un jugador con un PNJ **cedido**— no se puede montar en un navegador porque **no existe ninguna puerta para ceder un PNJ** (ficha P2-9), y vive en la prueba de componente con la API simulada. **Desde la tarea 8 del pulido (C2: #1)**, el gesto se pide abriendo antes «Más acciones sobre …» y eligiendo «Dar…» dentro — el botón «Dar» de la fila era el disparador viejo, y este recorrido es uno de los que lo cambió. |
...
- **El ataque no se ve resolverse PULSANDO «Atacar» en el navegador, de punta a punta.** El
  ataque contra la CA está medido en el servidor desde 2.5.3 (`ataque-comparado-en-el-servidor`) y
  el daño con su traza desde 2.5.4 (`dano-con-su-traza`); elegir objetivo entre los combatientes
  del encuentro está probado en RTL
  (`apps/web/src/features/character-sheet/__tests__/TirarAtaqueBoton.test.tsx`, 2026-09-05/06).
  **Desde la tarea 11 del pulido (C4, #15), `combate.spec.ts` sí mide que el resultado LLEGA al
  hilo real** —pedir el ataque y aplicar el daño citando su tirada por API, y comprobar que la
  página pinta «Thora ataca a Brann con … : impacta/falla» y «Brann pierde 3 PG (N → M) ← ataque de
  Thora»—, pero **monta el ataque por API, no pulsando el botón**. **Desde la tarea 9 de la
  puerta de efectos, `puerta-de-efectos.spec.ts` sí lo pulsa de punta a punta**: con un combate
  activo, elegir objetivo entre los combatientes de verdad en la lista del propio botón, resolver
  el ataque contra la CA (anulada a 1 para no depender de una tirada concreta) y tirar el daño
  desde el mismo panel — lo que sigue sin recorrido es ese mismo botón **encadenado con un
  encuentro que además tenga que gestionar bandos, o con un arma a dos manos**. `sesion.spec.ts`
  mide el gesto de anotar daño desde el elenco, pero con un número **escrito a mano**, no con el
  resultado de una tirada de ataque.
- **Curar a otro personaje desde una actividad, pulsando la pantalla.** `Actividades.tsx` —la
  única pantalla que usa una actividad del catálogo (`POST .../activities/:activityKey/use`)—
  nunca manda `objetivos`: no hay ningún selector de a quién apuntar, así que una actividad de
  salvación con `objetivos` (curación incluida) no se puede dirigir a otro personaje desde el
  navegador de hoy, sea lo que sea que el catálogo real exponga. El camino con objetivo por HTTP — que un
  jugador cure a otro y que la hoja del curado devuelva los PG cambiados — lo cubre
  `apps/api/test/puerta-de-efectos.e2e-spec.ts` contra la API real; **que ese cambio aparezca en la
  pantalla del curado sin recargar no lo cubre ningún recorrido**, porque un e2e de API no puede
  mirar una pantalla y ninguna pantalla puede hoy dirigir la actividad (tarea 9 de la puerta de
  efectos, recorrido (1) del brief, omitido por esta misma razón).
