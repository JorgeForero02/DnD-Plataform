...
describe("Comments (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    publicEntityId = (
      await request(s)
        ...
        .set("Authorization", `Bearer ${tokenDM}`)
        .send({ type: "NPC", name: "Open", visibility: "PLAYERS" })
    ).body.id;
    secretEntityId = (
      await request(s)
        .post(`/campaigns/${campaignId}/entities`)
        .set("Authorization", `Bearer ${tokenDM}`)
        .send({ type: "NPC", name: "Secret", visibility: "DM_ONLY" })
    ).body.id;
  });
  ...
  it("player comments on a visible entity and reads it back", async () => {
    const s = app.getHttpServer();
    const post = await request(s)
      .post(`/entities/${publicEntityId}/comments`)
      .set("Authorization", `Bearer ${tokenPL}`)
      .send({ body: "Nice NPC" });
    expect(post.status).toBe(201);
    const list = await request(s)
      .get(`/entities/${publicEntityId}/comments`)
      .set("Authorization", `Bearer ${tokenPL}`);
  ...
  it("player cannot comment on a DM_ONLY entity (403)", async () => {
    ...
    const res = await request(s)
      .post(`/entities/${secretEntityId}/comments`)
      .set("Authorization", `Bearer ${tokenPL}`)
      .send({ body: "sneaky" });
