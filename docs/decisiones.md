...
| D-CF-58 | **Reparto interno de tarjeta: cabecera · cuerpo · pie, relleno único, la rejilla coloca (no la tarjeta crece).** `e2e/espacios.spec.ts` mide `HUECO_MAX_PX` = 48 px y `DESNIVEL_MAX_PX` = 24 px; las cinco casillas de la cabecera pasan a `ANCHO_CASILLA_REM` = 4.75rem por `ALTO_CASILLA_REM` = 3.75rem, fijos y simétricos (anexo #3, #4, #6, #7, #9, #16, #17). Referencias: D&D Beyond (tarjetas con cabecera, números centrados). Tarea 0 del pulido, [nota de diseño](./superpowers/notes/2026-09-12-nota-de-diseno-ui-de-juegos.md) <!-- docs-lint-ignore --> |
