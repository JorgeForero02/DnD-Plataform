...
| D-CF-47 | **HP-9 —objetos mágicos con efecto— se hace DESPUÉS del paso 3, no antes; la sintonización de hoy es un marcador** (equipado + tope de tres + se pierde al quitar; el motor no la lee, no hay descanso corto y el SRD trae 0 objetos que la pidan). Razón: la mitad del catálogo mágico del SRD 5.1 concede conjuros, y sin el paso 3 se modelaría a medias; lo estructurable sin él son solo los +N. Estimación y alcance en su ficha de [06-pendientes.md](./06-pendientes.md); antes de abrir el plan, una spec con dos preguntas (¿solo +N o también resistencias y ventajas? ¿descanso corto real o un clic?). Decisión del autor, 2026-09-12 |
