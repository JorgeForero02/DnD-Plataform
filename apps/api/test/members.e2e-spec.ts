...
describe("Campaign members (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    tokenPL = (await request(s).post("/auth/register").send({ email: emailPL, password: "password123", displayName: "Player One" })).body.token;
    tokenOut = (await request(s).post("/auth/register").send({ email: emailOut, password: "password123", displayName: "Outsider" })).body.token;
    campaignId = (await request(s).post("/campaigns").set("Authorization", `Bearer ${tokenDM}`).send({ name: "C" })).body.id;
    const inv = (await request(s).post(`/campaigns/${campaignId}/invites`).set("Authorization", `Bearer ${tokenDM}`)).body.token;
  ...
  it("member lists members with displayName + role; outsider gets 403", async () => {
    const s = app.getHttpServer();
    const res = await request(s).get(`/campaigns/${campaignId}/members`).set("Authorization", `Bearer ${tokenPL}`);
    ...
    const forbidden = await request(s).get(`/campaigns/${campaignId}/members`).set("Authorization", `Bearer ${tokenOut}`);
