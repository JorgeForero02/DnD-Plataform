...
import { HojaDeVitela, PROSA_DE_VITELA, RotuloDeSeccion } from "./Vitela";
    ...
    <div className="flex flex-col gap-s4">
      ...
      >
        <div className="grid grid-cols-2 gap-s2 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <div className="rounded-radius-sm border border-muted bg-surface px-s3 py-s3 text-center">
        ...
        <div className="grid items-start gap-s5 lg:grid-cols-[minmax(0,7fr)_minmax(0,5fr)]">
          ...
          <div className="flex min-w-0 flex-col gap-s5">
            ...
            <section aria-label="valores pasivos" className="grid grid-cols-2 gap-s2">
              <ValorDerivado
                piel="vitela"
                etiqueta="Percepción pasiva"
                valor={sheet.derived.passivePerception}
              />
            </section>
          </div>
          ...
          <div className="flex min-w-0 flex-col gap-s4">
            ...
            <section aria-label="ataques y conjuros" className="grid grid-cols-2 gap-s2">
              ...
              <ValorDerivado
                piel="vitela"
                etiqueta="Ataque a distancia"
                valor={sheet.derived["attack.ranged"]}
              />
              {sheet.derived["attack.spell"] && (
                <ValorDerivado
                  piel="vitela"
                  etiqueta="Ataque de conjuro"
                  valor={sheet.derived["attack.spell"]}
                />
              )}
              ...
              <section aria-label="rasgos y aptitudes">
                ...
                <ul className="flex flex-wrap gap-s2">
                  {sheet.features.map((f) => (
                    <li
                      key={f.labelKey}
                      className="rounded-radius-sm border border-copper px-s2 py-1 font-world text-[length:var(--text-world-sm)] text-text"
                    >
                      {f.name}
                    </li>
                  ))}
