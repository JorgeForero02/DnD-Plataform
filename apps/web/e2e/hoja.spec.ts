...
test("H3/H5 — la cabecera se queda fija al desplazar, y un paso de la traza lleva el foco a su causa", async ({
  ...
  await expect(cabecera.locator('[data-casilla="derivada"]')).toHaveCount(5);
...
test("las cinco casillas de la tira miden lo mismo, con y sin temporales (anexo #4)", async ({
  ...
  // Por dato, no por clase: un cambio de ancho no debe romper el test (revisión 2026-09-13).
  const cajas = await tira.locator('[data-casilla="derivada"]').all();
