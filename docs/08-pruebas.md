...
| `puerta-de-efectos` | **Tarea 9 de la puerta de efectos (spec 2026-09-12), tres recorridos con dos navegadores, cada uno con un `expect` que mira el navegador del OTRO contexto.** (2) La condición «hasta el próximo descanso largo»: el DM se la aplica al personaje de B con el radio nuevo (`Condiciones.tsx`) y **la hoja de B ya lo dice sin que B haya tocado nada**; un descanso corto no la toca, uno largo la retira, y el hilo de la sesión traduce `REST_DECLARED` como «Descanso largo». (3) La bandeja de daño (`BandejaDeDano.tsx`): con la CA de un goblin anulada a 1 desde «Anulaciones del DM» (para no depender de una tirada de ataque concreta), A lo ataca y tira el daño desde su cuadro de ataques; en el hilo del DM la tarjeta trae «Aplicar el daño a Goblin» y en el de A solo «Daño pendiente» —el mismo 404 que un objetivo que no se ve—; al aplicar, **los dos** ven «Aplicado» sin recargar y los PG del goblin bajan en el elenco (`role="img"` con su `aria-label`). (4) El XP: con la regla de la mesa en «Por experiencia», la hoja de A dice «0 / 300 PX»; el DM da 300 desde «Dar XP» y **el marcador de A sube a «300 / 300 PX» con el aviso de nivel 2 sin que A haya pedido nada**, y el nivel de A, comprobado contra la API, se queda en 1. **El recorrido (1) del brief —un clérigo cura a un guerrero con una actividad de salvación— se omite**: `Actividades.tsx`, la única pantalla que usa una actividad del catálogo, manda `usar.mutate({ activityKey })` sin ningún campo `objetivos` — no hay selector de objetivo en esa pantalla, así que ninguna actividad con salvación se puede dirigir a otro personaje desde el navegador hoy, sea lo que sea que exponga el catálogo. Ese camino lo cubre `apps/api/test/puerta-de-efectos.e2e-spec.ts` contra la API real. **No se corre en esta ficha** (D-CF-65): la ejecuta el orquestador al cerrar. |
...
- **Curar a otro personaje desde una actividad, pulsando la pantalla.** `Actividades.tsx` —la
  ...
  navegador de hoy, sea lo que sea que el catálogo real exponga. El camino con objetivo — que un
  jugador cure a otro y que los PG del curado cambien en su propia pantalla sin recargar — lo
  cubre `apps/api/test/puerta-de-efectos.e2e-spec.ts` contra la API real (tarea 9 de la puerta de
  efectos, recorrido (1) del brief, omitido aquí por esta misma razón).
