...
| `tablero-en-la-mesa` | **C1 bis (2026-09-12, D-CF-63): el tablero PlanarAlly enmarcado en el centro de la mesa, con hilo LARGO (doce anotaciones, como `mesa-mide`).** Con `boardRoomUrl` guardada y el cajón desplegado, cinco medidas —no es «la misma de `mesa-mide`»—: (1) la raíz `h-screen overflow-hidden` no scrollea (vacía por construcción, se deja por paridad); (2) **el tablero manda sobre el registro** (decisión del autor, ronda de revisión 2): el `<iframe>` nunca es más bajo que el propio cajón abierto, y además mide al menos un cuarto de la altura de la ventana (200 px a 800×; el cajón está topado en `max-h-[32vh]` = 256 px, bajado desde 40vh porque a 1280×800 la banda de escena y la línea de combate ya se comen ~160 px por encima de la rejilla) —no se ha desplomado a 0 px por un hilo largo—; (3) el hilo, dentro del cajón, scrollea **por dentro** (`overflowY` resuelto y `scrollHeight > clientHeight` sobre la lista «Sucesos de la sesión», la misma sonda de `mesa-mide`); (4) el compositor «Qué anotar» sigue dentro de la ventana, sin que el `overflow-hidden` de la raíz lo recorte. Plegar el registro y sellar una nota **por API** (`POST .../sessions/notes`, porque con el registro plegado el compositor «Anotar» no está montado) sube su contador, y desplegarlo lo vuelve a cero y enseña el hilo de siempre. La «sala» de la prueba es la propia `/acerca-de` (mismo origen: la web no manda `X-Frame-Options` contra sí misma), así que no sale a internet. Una segunda prueba monta la mesa entera a 1280×800 —donde sí funciona— y **solo entonces** reduce la ventana a 390 px para comprobar el orden: el marco arriba y el cajón debajo. **Lo que esta segunda prueba NO demuestra**: que la columna central quepa o se lea a 390 px — la mesa a ese ancho sigue aplazada (D-CF-26), y solo se mide el orden dentro de una columna que hoy queda estrujada. |
